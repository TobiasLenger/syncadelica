...
const musicService_1 = require("./musicService"); // Import fetchSongDetails
...
app.get('/api/song-details', async (req, res) => {
    ...
    try {
        ...
        const details = await (0, musicService_1.fetchSongDetails)(pageUrl, undefined, artist, title);
...
app.get('/api/stream', async (req, res) => {
    const directAudioUrl = req.query.url; // Changed from pageUrl to url
    if (!directAudioUrl) {
    ...
    try {
        ...
        if (range) {
            ...
            const audioRes = await (0, node_fetch_1.default)(downloadUrl, {
                headers: {
                    'Range': range,
